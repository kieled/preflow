...
function PreflowGridView({ count }: { count: number }) {
	...
	const colW = scrollRef.current
		? (scrollRef.current.clientWidth - GAP * (cols - 1)) / cols
		: MIN_COL;
	const getHeight = useCallback((i: number) => heights(i), []);
...
function TanStackGridView({ count }: { count: number }) {
	...
	useEffect(() => {
		...
		if (!el) return;
		const ro = new ResizeObserver((e) =>
			setCols(Math.max(1, Math.floor((e[0]!.contentRect.width + GAP) / (MIN_COL + GAP)))),
		);
		ro.observe(el);
		return () => ro.disconnect();
	}, []);
	const colW = parentRef.current
		? (parentRef.current.clientWidth - GAP * (cols - 1)) / cols
		: MIN_COL;
	const v = useVirtualizer({
		count,
		getScrollElement: () => parentRef.current,
		...
		<div className="example-viewport">
			<Virtuoso
				...
				itemContent={(index) => (
					<div
						className="flow-item"
						style={{ height: heights(index), backgroundColor: seededColor(index) }}
					>
...
function VirtuosoGridView({ count }: { count: number }) {
	return (
		<div className="example-viewport">
			...
			<VirtuosoGrid
				totalCount={count}
				listClassName="vg-list"
				itemClassName="vg-item"
				style={{ height: "100%" }}
				itemContent={(index) => (
					<div
						className="grid-item"
						style={{ backgroundColor: seededColor(index), height: "100%", borderRadius: 4 }}
					>
						<div style={{ textAlign: "center" }}>
	...
	return (
		<div className="example-layout">
			<div className="example-controls">
				<h3>Stress Test</h3>
				<p>
					Compare scroll and grid performance across libraries. Switch between Preflow, TanStack
					Virtual, and Virtuoso in list or auto-column grid mode.
				</p>
				<div style={{ marginTop: 8 }}>
					...
					<div className="tab-bar">
						<button
							type="button"
							className={lib === "preflow" ? "active" : ""}
							onClick={() => setLib("preflow")}
						>
							Preflow
						</button>
						<button
							type="button"
							className={lib === "tanstack" ? "active" : ""}
							onClick={() => setLib("tanstack")}
						>
							TanStack
						</button>
						<button
							type="button"
							className={lib === "virtuoso" ? "active" : ""}
							onClick={() => setLib("virtuoso")}
						>
							Virtuoso
						</button>
					</div>
					...
					<div className="tab-bar">
						{(Object.keys(COUNTS) as CountKey[]).map((k) => (
							<button
								type="button"
								key={k}
								className={countKey === k ? "active" : ""}
								onClick={() => setCountKey(k)}
							>
								{k}
							</button>
						))}
